...
interface EnemyDisconnectedOverlayProps {
  result: "win" | "lose"; // added prop
  onHome?: () => void;
  onMatching?: () => void;
}

export default function EnemyDontMoveOverlay({
  result,
  onHome,
  onMatching,
  ...
  const message =
    result === "win"
      ? "Your enemy didn't move in time."
      : "You didn't move in time.";

  const colorA = result === "win" ? "#2E8B57" : "#B22222";
  const colorB = result === "win" ? "#1F6A40" : "#8B1A1A";
