...
import type { IPokemonBattle } from "../db/pokemonBattle";
import type { IBattlePokemon } from "../db/battle.js";
...
export const setupDummyBattleData = async () => {
  try {
    const findPokemon = async (name: string): Promise<IPokemonBattle | null> => {
      const p = await PokemonBattle.findOne({
